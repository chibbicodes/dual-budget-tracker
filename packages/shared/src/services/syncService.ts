 ...
 *
 * This version uses injected adapters for database and storage,
 * making it portable across different environments (web, mobile, etc.)
 */
...
export interface DatabaseAdapter {
  ...
  getCategories(profileId: string): Promise<any[]>
  ...
  getTransactions(profileId: string): Promise<any[]>
  ...
  getIncomeSources(profileId: string): Promise<any[]>
  ...
  deleteIncomeSource(id: string): Promise<any>
  ...
  getProjectTypes(profileId: string): Promise<any[]>
  ...
  deleteProjectType(id: string): Promise<any>
...
class SyncService {
  ...
  constructor(db: DatabaseAdapter, storage: StorageAdapter) {
    this.db = db
  ...
  private async syncAccounts(profileId: string): Promise<void> {
    try {
      ...
      const accounts = await this.db.getAccountsForSync(profileId)
  ...
  private async syncCategories(profileId: string): Promise<void> {
    try {
      ...
      const categories = await this.db.getCategoriesForSync(profileId)
  ...
  private async syncTransactions(profileId: string): Promise<void> {
    try {
      const transactions = await this.db.getTransactionsForSync(profileId)
  ...
  private async syncIncomeSources(profileId: string): Promise<void> {
    try {
      const incomeSources = await this.db.getIncomeSourcesForSync(profileId)
      ...
      for (const source of incomeSources) {
        await syncRecordToCloud('incomeSources', {
          ...
          firstOccurrenceAmount: source.first_occurrence_amount || null,
          frequency: source.frequency,
          nextExpectedDate: source.next_expected_date,
          endCondition: source.end_condition || 'none',
          endDate: source.end_date || null,
          totalOccurrences: source.total_occurrences || null,
  ...
  private async syncProjects(profileId: string): Promise<void> {
    try {
      const projects = await this.db.getProjectsForSync(profileId)
  ...
  private async syncProjectTypes(profileId: string): Promise<void> {
    try {
      const projectTypes = await this.db.getProjectTypesForSync(profileId)
  ...
  private async syncProjectStatuses(profileId: string): Promise<void> {
    try {
      const projectStatuses = await this.db.getProjectStatusesForSync(profileId)
  ...
  private async pullAccounts(profileId: string): Promise<void> {
    try {
      ...
      for (const cloudAccount of cloudAccounts) {
        ...
        const localAccount = await this.db.getAccount(cloudAccount.id)
        ...
        if (
          ...
          const existingAccount = await this.db.getAccountForSync(cloudAccount.id)

          if (existingAccount) {
            // Update existing account including deleted_at field
            await this.db.updateAccountForSync(cloudAccount.id, {
  ...
  private async pullCategories(profileId: string): Promise<void> {
    try {
      ...
      for (const cloudCategory of cloudCategories) {
        ...
        const localCategory = await this.db.getCategory(cloudCategory.id)
        ...
        if (
          ...
          const existingCategory = await this.db.getCategoryForSync(cloudCategory.id)

          if (existingCategory) {
            // Update existing category including deleted_at field
            await this.db.updateCategoryForSync(cloudCategory.id, {
            ...
            // Create new category
            await this.db.createCategoryForSync({
              id: cloudCategory.id,
              profileId: profileId,
              name: cloudCategory.name,
              budgetType: cloudCategory.budgetType,
  ...
  private async pullTransactions(profileId: string): Promise<void> {
    try {
      ...
      for (const cloudTransaction of cloudTransactions) {
        ...
        const localTransaction = await this.db.getTransactionForSync(cloudTransaction.id)
        ...
        if (
          ...
          if (localTransaction) {
            ...
            await this.db.updateTransaction(cloudTransaction.id, {
  ...
  private async pullIncomeSources(profileId: string): Promise<void> {
    try {
      ...
      for (const cloudIncomeSource of cloudIncomeSources) {
        ...
        const localIncomeSource = await this.db.getIncomeSourceForSync(cloudIncomeSource.id)
        ...
        if (
          ...
          if (localIncomeSource) {
            // Update existing, preserving cloud deletion status
            await this.db.updateIncomeSource(cloudIncomeSource.id, {
              ...
              first_occurrence_amount: cloudIncomeSource.firstOccurrenceAmount || null,
              frequency: cloudIncomeSource.frequency,
              next_expected_date: cloudIncomeSource.nextExpectedDate,
              end_condition: cloudIncomeSource.endCondition || 'none',
              end_date: cloudIncomeSource.endDate || null,
            ...
            // Only create locally if the cloud record is not deleted
            await this.db.createIncomeSource({
  ...
  private async pullProjects(profileId: string): Promise<void> {
    try {
      ...
      for (const cloudProject of cloudProjects) {
        ...
        const localProject = await this.db.getProjectForSync(cloudProject.id)
        ...
        if (
          ...
          if (localProject) {
            // Update existing, preserving cloud deletion status
            await this.db.updateProject(cloudProject.id, {
  ...
  private async pullProjectTypes(profileId: string): Promise<void> {
    try {
      ...
      for (const cloudProjectType of cloudProjectTypes) {
        ...
        const localProjectType = await this.db.getProjectTypeForSync(cloudProjectType.id)
        ...
        if (
          ...
          if (localProjectType) {
            // Update existing, preserving cloud deletion status
            await this.db.updateProjectType(cloudProjectType.id, {
            ...
            // Only create locally if the cloud record is not deleted
            await this.db.createProjectType({
  ...
  private async pullProjectStatuses(profileId: string): Promise<void> {
    try {
      ...
      for (const cloudProjectStatus of cloudProjectStatuses) {
        ...
        const localProjectStatus = await this.db.getProjectStatusForSync(cloudProjectStatus.id)
        ...
        if (
          ...
          if (localProjectStatus) {
            // Update existing, preserving cloud deletion status
            await this.db.updateProjectStatus(cloudProjectStatus.id, {
            ...
            // Only create locally if the cloud record is not deleted
            await this.db.createProjectStatus({
    ...
    try {
      ...
      for (const account of allAccounts as any[]) {
        if (!validProfileIds.has(account.profile_id)) {
          ...
          await this.db.deleteAccount(account.id)
        }
      }

      // Get all categories and delete those with invalid profile_id
      const allCategories = await this.db.getCategories('')
      ...
      for (const transaction of allTransactions as any[]) {
        if (!validProfileIds.has(transaction.profile_id)) {
          ...
          await this.db.deleteTransaction(transaction.id)
        }
      }

      // Get all income sources and delete those with invalid profile_id
      const allIncomeSources = await this.db.getIncomeSources('')
      for (const source of allIncomeSources as any[]) {
        if (!validProfileIds.has(source.profile_id)) {
          ...
          await this.db.deleteIncomeSource(source.id)
        }
      }
      ...
      const allProjectTypes = await this.db.getProjectTypes('')
      for (const type of allProjectTypes as any[]) {
        if (!validProfileIds.has(type.profile_id)) {
          ...
          await this.db.deleteProjectType(type.id)
        }
      }

      // Get all project statuses and delete those with invalid profile_id
      const allProjectStatuses = await this.db.getProjectStatuses('')
      for (const status of allProjectStatuses as any[]) {
        if (!validProfileIds.has(status.profile_id)) {
          ...
          await this.db.deleteProjectStatus(status.id)
        }
      }
  ...
  getLastSyncedAt(): Date | null {
    const timestamp = this.storage.getItem('lastSyncedAt')
  ...
  getAutoSyncEnabled(): boolean {
    return this.storage.getItem('autoSyncEnabled') === 'true'
  }
  ...
  setAutoSyncEnabled(enabled: boolean): void {
    this.storage.setItem('autoSyncEnabled', enabled ? 'true' : 'false')
  }
...
export function createSyncService(db: DatabaseAdapter, storage: StorageAdapter): SyncService {
  return new SyncService(db, storage)
}
